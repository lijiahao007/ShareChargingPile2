...
class ChargingPileListFragment : Fragment() {
    ...
    private fun initUI(){
        ...
        val singlePiles = ArrayList<ChargingPile>() // 每个ChargingPile分来，例如2个直流12kw/h 变成 1个直流12kw/h, 1个直流12kw/h
        piles?.forEach {
            for (i in 0 until it.pileNum) {
                val pile = ChargingPile(it.id, it.electricType, it.powerRate, 1, it.stationId, it.state)
